...
namespace huadb {
    ...
    InsertLog::InsertLog(lsn_t lsn, xid_t xid, lsn_t prev_lsn, oid_t oid, pageid_t page_id, slotid_t slot_id,
        ...
        size_ +=
                sizeof(oid_) + sizeof(page_id_) + sizeof(slot_id_) + sizeof(page_offset_) + sizeof(record_size_) +
                record_size_;
    }
    ...
    std::shared_ptr<InsertLog> InsertLog::DeserializeFrom(lsn_t lsn, const char *data) {
        ...
        offset += record_size;
        auto log = std::make_shared<InsertLog>(lsn, xid, prev_lsn, oid, page_id, slot_id, page_offset, record_size,
                                               record);
        delete[] record;
        return log;
    }
