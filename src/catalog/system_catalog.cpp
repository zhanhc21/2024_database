...
namespace huadb {
    ...
    void
    SystemCatalog::CreateTable(const std::string &table_name, const ColumnList &column_list, oid_t oid, oid_t db_oid,
                               bool new_table) {
        // Step 1. 约束检测
        if (db_oid == INVALID_OID) {
            CheckUsingDatabase();
    ...
    void SystemCatalog::LoadTableMeta() {
        ...
        while (auto record = scan->GetNextRecord()) {
            if (record->GetValue(db_oid_idx).GetValue<oid_t>() == current_database_oid_) {
                ...
                // 添加数据表
                oid_manager_.SetEntryOid(OidType::TABLE, table_name, oid);
                oid2table_[oid] = std::make_shared<Table>(buffer_pool_, log_manager_, oid, current_database_oid_,
                                                          column_list,
                                                          false,
                                                          Disk::EmptyFile(Disk::GetFilePath(GetDatabaseOid(oid), oid)));
                table2cardinality_[table_name] = cardinality;
            }
        }
    }
