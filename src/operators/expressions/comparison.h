...
namespace huadb {
    ...
    public:
        Comparison(ComparisonType type, std::shared_ptr<OperatorExpression> left,
                   std::shared_ptr<OperatorExpression> right)
                : OperatorExpression(OperatorExpressionType::COMPARISON, {std::move(left), std::move(right)},
                                     Type::BOOL),
                  type_(type) {}
